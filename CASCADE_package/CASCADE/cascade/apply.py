...
from nfp.preprocessing.preprocessor import MolAPreprocessor
from nfp.preprocessing.sequence import GraphSequence
from cascade.genConf import genConf
...
def preprocess_C(mols, preprocessor, keep_all_cf=False):
    ...
    for input in inputs:
        input.pop('bond', None)
        input.pop('node_graph_indices', None)
...
def predict_NMR_C(ID, model, dir_path):

    mols = [Chem.MolFromMolFile(ID + '.sdf')]
    for m in mols: AllChem.EmbedMolecule(m, useRandomCoords=True)
    mols = [Chem.AddHs(m, addCoords=True) for m in mols]


    with open(dir_path + '/cascade/preprocessor.p', 'rb') as ft:
...
def predict_NMR_H(ID, model, dir_path):
