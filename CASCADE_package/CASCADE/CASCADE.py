...
def prediction(save_folder, path_csv):
    ...
    modelpath_C = dir_path + '/cascade/trained_model/best_model.hdf5'
    modelpath_H = dir_path + '/cascade/trained_model/best_model_H_DFTNN.hdf5'
    ...
    for ID in data.id:
        ...
        mols, weightedPrediction, spreadShift = predict_NMR_C(ID, NMR_model_C, dir_path)
        weightedPrediction['ID'] = ID
        pred_data_C = pd.concat([pred_data_C,weightedPrediction])

        # H predictions
        pred_data_H = pd.DataFrame()
        mols, weightedPrediction, spreadShift = predict_NMR_H(ID, NMR_model_H, dir_path)
        weightedPrediction['ID'] = ID
